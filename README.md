...
scheduled **GitHub Action** refreshes it twice every weekday (after the close
and again before the open, so an overnight earnings report is on the site the
next morning) and deploys to **GitHub Pages**.
...
net income, EPS, the analyst consensus, and the beat/miss surprise). These
extras are generated into `data/details.json` (keyed by symbol) by
`generate_data.py`, alongside the last ~120 daily closes for the chart.

### Earnings freshness

The quarterly **income statement** Yahoo exposes is filled from the company's
*filed* financials, which land days to weeks after the press release — on its
own it makes a stock look like it never reported. So earnings are assembled
from three sources and merged in `generate_data.py`:

| Source | Carries | Lag |
|---|---|---|
| `earningsHistory` (quoteSummary) | headline EPS, consensus estimate, surprise % | same day as the report |
| `calendarEvents` (quoteSummary) | next report date + consensus EPS / revenue | rolls forward within a day |
| quarterly income statement | revenue, net income, diluted EPS | days to weeks (SEC filing) |

A quarter that has been **reported but not yet filed** shows up immediately,
tagged *just reported*, with its EPS and surprise but no revenue or net income
yet; the filed numbers fill in on a later run. The next-report date prefers the
calendar (which updates the day after a report) over the `earningsTimestamp`
fields in `.info` (which can lag), drops dates that are already in the past, and
marks the date `(est.)` when Yahoo has only projected it.

Two things protect that data from Yahoo's rate limiting, which is the other
reason a ticker can look stale:

- Rate-limited (HTTP 429) fundamentals calls are **retried with backoff**
  instead of silently returning nothing, and the Action logs how many tickers
  ended up without info, without history, and without a report date.
- The workflow hands the generator the `details.json` the site is **currently
  serving** (`--previous`), so any quarter or report date this run failed to
  fetch is carried over from the last good one rather than published as a gap.
...
- Generated by `scripts/generate_rvol_scan.py` into `data/rvol_scan.json`; the
  GitHub Action runs it after each close alongside the main dashboard. If the
  symbol directory is unreachable it falls back to the site's existing universe.
...
├── requirements.txt
└── .github/workflows/
    ├── update-and-deploy.yml  # twice-daily refresh + Pages deploy
...
- on a weekday schedule **twice a day** — 22:00 UTC (after the US close, which
  catches same-evening earnings) and 10:30 UTC (before the open, which catches
  anything reported after the previous close),
- and manually via **workflow_dispatch**.

The pre-market run skips the market-wide RVOL scan — it is the slowest step and
has no new volume to measure before the open — and reuses the `rvol_scan.json`
the live site is already serving, falling back to a real scan if that file
can't be fetched.
